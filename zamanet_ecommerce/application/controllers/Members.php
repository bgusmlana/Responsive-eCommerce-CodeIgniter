...
class Members extends CI_Controller
{
	...
	function dashboard()
	{
		...
		$data['record'] = $this->db->get_where('tb_pengguna', "id_pengguna='$id'")->row_array();
		$data['rows'] = $this->model_app->alamat_konsumen($id)->row_array();
	...
	function edit_profile()
	{
		...
		if (isset($_POST['submit'])) {
			...
			$row = $this->model_app->profile_konsumen($this->session->id_pengguna)->row_array();
	...
	function edit_alamat()
	{
		...
		$id = $this->uri->segment(3);
		if (isset($_POST['submit'])) {
			$this->model_members->alamat_update($this->session->id_pengguna);
			redirect('members/edit_alamat');
		} else {
			$data['title'] = 'Ubah Alamat Saya';
			$data['breadcrumb'] = 'Ubah Alamat';
			$data['row'] = $this->model_app->alamat_konsumen($this->session->id_pengguna)->row_array();
			$data['kota'] = $this->model_app->view('tb_kota');
